...
var _ = Describe("KMM", Ordered, Label(kmmparams.LabelSuite, kmmparams.LabelSanity), func() {
	...
	Context("Module", Label("multiple"), func() {
		...
		AfterAll(func() {
			...
			err = await.ModuleObjectDeleted(APIClient, kmodName, nSpace, 5*time.Minute)
		...
		Context("Modprobe", Label("multiple"), func() {
			...
			It("should fail if any of the modules is not present", reportxml.ID("62743"), func() {
				...
				err = await.BuildPodCompleted(APIClient, nSpace, 25*time.Minute)
				Expect(err).ToNot(HaveOccurred(), "error while building module")

				By("Await driver container deployment")
				err = await.ModuleDeployment(APIClient, kmodName, nSpace, 23*time.Minute,
					GeneralConfig.WorkerLabelMap)
				Expect(err).ToNot(HaveOccurred(), "error while waiting on driver deployment")

				By("Check module is loaded on node")
				err = check.ModuleLoaded(APIClient, kmodName, 25*time.Minute)
				Expect(err).ToNot(HaveOccurred(), "error while checking the module is loaded")

				By("Check module is loaded on node")
				err = check.ModuleLoaded(APIClient, "kmm-ci-a", 25*time.Minute)
				Expect(err).ToNot(HaveOccurred(), "error while checking the module is loaded")
