...
var _ = Describe("KMM", Ordered, Label(kmmparams.LabelSuite, kmmparams.LabelSanity), func() {
	...
	Context("Module", Label("simple-kmod"), func() {
		...
		It("should build and push image to quay", reportxml.ID("53584"), func() {
			...
			err = await.ModuleDeployment(APIClient, moduleName, localNsName, 15*time.Minute, GeneralConfig.WorkerLabelMap)
		...
		It("should delete simple-kmod module", reportxml.ID("53413"), func() {
			...
			err = await.ModuleObjectDeleted(APIClient, kmodName, localNsName, 15*time.Minute)
		...
		It("should delete simple-kmod module", reportxml.ID("53413"), func() {
			...
			By("Await module to be deleted")
			err = await.ModuleObjectDeleted(APIClient, kmodName, localNsName, 25*time.Minute)
			Expect(err).ToNot(HaveOccurred(), "error while waiting module to be deleted")
