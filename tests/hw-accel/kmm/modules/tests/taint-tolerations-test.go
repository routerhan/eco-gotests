...
var _ = Describe("KMM", Ordered, Label(kmmparams.LabelSuite, kmmparams.LabelSanity), func() {
	...
	Context("Module", Label("tolerations"), func() {
		...
		AfterEach(func() {
			...
			err = await.ModuleObjectDeleted(APIClient, moduleName, kmmparams.TolerationModuleTestNamespace, 5*time.Minute)
		...
		It("should deploy module with NoSchedule toleration", reportxml.ID("79205"), func() {
			...
			err = await.BuildPodCompleted(APIClient, kmmparams.TolerationModuleTestNamespace, 25*time.Minute)
			Expect(err).ToNot(HaveOccurred(), "error while building module")

			By("Await driver container deployment")
			err = await.ModuleDeployment(APIClient, moduleName, kmmparams.TolerationModuleTestNamespace, 20*time.Minute,
		...
		It("should deploy module with NoExecute toleration", reportxml.ID("79206"), func() {
			...
			By("Await driver container deployment")
			err = await.ModuleDeployment(APIClient, moduleName, kmmparams.TolerationModuleTestNamespace, 5*time.Minute,
				GeneralConfig.WorkerLabelMap)
			Expect(err).ToNot(HaveOccurred(), "error while waiting on driver deployment")
		...
		It("should deploy module with device plugin and NoSchedule toleration", reportxml.ID("79997"), func() {
			...
			err = await.DeviceDriverDeployment(APIClient, moduleName, kmmparams.TolerationModuleTestNamespace, 5*time.Minute,
				GeneralConfig.WorkerLabelMap)
